    ...
    fun getFilePath(formatNmae: String): File? {
        ...
        when (formatNmae) {
            "mp4", "h264", "3gp" -> {//保存为视频文件
                filePath = File("${DefaultValue.ROOT_DIR}/${FinalValue.APP_NAME}/videos/${time}.${formatNmae}");
            }
            "jpg", "JPEG", "png", "PNG" -> {//保存为图片文件
                filePath = File("${DefaultValue.ROOT_DIR}/${FinalValue.APP_NAME}/images/${time}.${formatNmae}");
            }
            "aac" -> {//保存为声音文件
                filePath = File("${DefaultValue.ROOT_DIR}/${FinalValue.APP_NAME}/voices/${time}.${formatNmae}");
            }
            else -> {//保存为其他文件
                filePath = File("${DefaultValue.ROOT_DIR}/${FinalValue.APP_NAME}/elses/${time}.${formatNmae}");
            }
        }
        creatFile(filePath)
        return filePath;
